  ...
  std::string chessState::searchMove(){
    ...
    for(auto c: moves){
      if(legalMove(c)){
        legalMoves.push_back(c);
      }
    }

    if(legalMoves.size() <= 0){
      int kingPos = __builtin_ctzll(bitboards[active][KING]);
      if(isThreatenedBit(kingPos, occupied[0] | occupied[1], active)){
        return "Checkmate";
      }
      return "Draw";
    }
  ...
  int chessState::evaluationHeuristic(chessState& state){
    ...
    for(int c = 0; c < 2; c++){
      if(c == state.active){
        ...
        if((left == right) && left){ //three move repretition check
          score -= 200000;
        }
      }
      
      uint64_t aBoard = state.occupied[c], eBoard = fBoard & ~aBoard & ~bitboards[!c][PAWN]; //active board, enemy board disregarding their pawns
      uint64_t kingBit = state.bitboards[!c][KING];
      ...
      for(int p = 0; p < 6; p++){
        ...
        score += sign * __builtin_popcountll(pieces) * pieceValues[p];
        if(p != KING){

          while(pieces){
            int pos = __builtin_ctzll(pieces);
            uint64_t mask = retrieveAttackBoard(pos, static_cast<Color>(c), static_cast<PieceType>(p), fBoard); //current pieces attack mask
            mask &= ~(aBoard); //marks friendly spaces as inaccessible
            int moveCount = __builtin_popcountll(mask);  //adds slight value for pieces being able to control more space
            score += sign * 2 * moveCount;
            moveCount = __builtin_popcountll(mask & enemyKingCoverage); //adds value for threatening spaces adjacent to king
            pressure += 2 * moveCount;
            moveCount = __builtin_popcountll(mask & eBoard); //adds value for threatening non pawn pieces
            score += sign * 2 * moveCount;
            moveCount = __builtin_popcountll(mask & kingBit); //adds value for holding enemy king in check
            check = (mask & kingBit) || check; //adds bonus for check 
            enemyKingCoverage &= ~mask; //only considers kings adjacent space once for scoring
            eBoard &= ~mask; //only considers each piece once for scoring pressure
            moveCount = __builtin_popcountll(mask & state.bitboards[c][KING]);
